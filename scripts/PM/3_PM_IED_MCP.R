...
converted_data_raw <- read_xlsx("data/IED_data.xlsx", sheet = "PM_from_EMEPEEA") |> 
  select(`Source[1]`:`...10`) 
...
ggplot(converted_data) +
  ...
  geom_smooth(aes(x=ELV_mgNm3, y = EF_g_kwh,  colour = fuel_types), linetype = "dashed", linewidth = 1.2, method = "lm") +
...
ggsave(filename = "plots/supp_info/conversion_charts_pm.png", device = "png", height = 20, width = 20, units = "cm")
...
mcp_ied_standards <- read_csv("data/MCP_IED_standards_for_conversion_PM.csv") |> 
  pivot_longer(cols = c(`Gas Turbine`:`Coal Boiler`), names_to = "CP_type", values_to = "PM_ELV_mg_Nm3") |> 
  mutate(efficiency = ifelse(grepl("Gas", CP_type, ignore.case = TRUE), 0.5, 0.36)) |> 
  mutate(conversion_factor = case_when(grepl("Gas", CP_type, ignore.case = TRUE) ~ 992,
                                       grepl("Coal", CP_type, ignore.case = TRUE) ~ 767,
                                       grepl("Biomass", CP_type, ignore.case = TRUE) ~ 722))


# Now apply the conversions..... 

mcp_ied_standards_with_conversions_PM <- mcp_ied_standards |> 
  mutate(converted_pm_g_kwh = PM_ELV_mg_Nm3/ (conversion_factor * efficiency)) |> 
  mutate(converted_pm_g_kwh_max_efficiency = PM_ELV_mg_Nm3/ (conversion_factor * 0.95)) |> 
  mutate(converted_pm_g_kwh_min_efficiency = PM_ELV_mg_Nm3/ (conversion_factor * 0.30)) |> 
  mutate(max_power = ifelse(is.na(max_power), 3000, max_power), 
         avg_power = (max_power + min_power)/2)


ggplot(mcp_ied_standards_with_conversions_PM) +
  geom_point(aes(x = avg_power * 1000, y = converted_pm_g_kwh, colour = CP_type)) +
  geom_errorbarh(aes(xmin = min_power * 1000, xmax = max_power * 1000, y = converted_pm_g_kwh), colour = "grey") +
  geom_pointrange(aes(ymin = converted_pm_g_kwh_min_efficiency, ymax = converted_pm_g_kwh_min_efficiency, x = avg_power * 1000, y = converted_pm_g_kwh), colour = "grey") +
  geom_line(aes(x = avg_power * 1000, y = converted_pm_g_kwh, colour = CP_type)) +
  geom_line(aes(x = avg_power * 1000, y = converted_pm_g_kwh_max_efficiency, colour = CP_type)) +
  geom_line(aes(x = avg_power * 1000, y = converted_pm_g_kwh_min_efficiency, colour = CP_type)) +
  scale_x_continuous(trans = "log10") +
  facet_wrap(~CP_type, scales = "free")


write.csv(mcp_ied_standards_with_conversions_PM, file = "data/mcp_ied_standards_with_conversions_PM.csv")

# And converting to the standard table format 


collated_data_pm <- mcp_ied_standards_with_conversions_PM |> 
  select(c(directive, converted_pm_g_kwh, avg_power, CP_type)) |> 
  mutate(converted_pm_mg_kwh = converted_pm_g_kwh * 1000, avg_power_kw = avg_power * 1000) |> 
  rename(Directive = directive, Product = CP_type, `Power (kW)` = avg_power_kw,  `PM (mg/kWh)` = converted_pm_mg_kwh) |> 
  mutate(Fuel = word(Product, 1)) |> 
  filter(is.na(`PM (mg/kWh)`) == F) |> 
  select(-c(converted_pm_g_kwh, avg_power)) |> 
